...
defineProps<{ cloverAccounts: CloverAccountRow[] }>();
...
const deactivateTarget = ref<CloverAccountRow | null>(null);
const deactivateOpen   = ref(false);
const deactivateForm   = useForm({});

const activateTarget = ref<CloverAccountRow | null>(null);
const activateOpen   = ref(false);
const activateForm   = useForm({});

const deleteTarget = ref<CloverAccountRow | null>(null);
const deleteOpen   = ref(false);
const deleteForm   = useForm({});

function confirmActivate(account: CloverAccountRow) {
...
function executeActivate() {
    ...
    activateForm.patch(`/admin/clover-accounts/${activateTarget.value.id}/activate`, {
        onSuccess: () => {
...
function confirmDeactivate(account: CloverAccountRow) {
...
function executeDeactivate() {
    ...
    deactivateForm.patch(`/admin/clover-accounts/${deactivateTarget.value.id}/deactivate`, {
        onSuccess: () => {
...
function confirmDelete(account: CloverAccountRow) {
...
function executeDelete() {
    ...
    deleteForm.delete(`/admin/clover-accounts/${deleteTarget.value.id}`, {
        onSuccess: () => {
...
<template>
    ...
    <div class="p-6 space-y-6">
        <div class="flex items-center justify-between">
            ...
            <Button as-child>
                <Link href="/admin/clover-accounts/create">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-[#F7F5F2] border-b border-border">
                        ...
                        <th class="text-left px-5 py-3.5 text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">Merchant ID</th>
                ...
                <tbody>
                    <tr
                        v-for="account in cloverAccounts"
                        :key="account.id"
                        ...
                        <td class="px-5 py-3.5 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <Button variant="ghost" size="icon" as-child>
                                    <Link
                                        :href="`/admin/clover-accounts/${account.id}/edit`"
                    ...
                    <tr v-if="cloverAccounts.length === 0">
                        <td colspan="6" class="px-5 py-16 text-center text-muted-foreground text-sm">
                            No Clover accounts yet. Add an account to enable USD payment collection via Clover.
                        </td>
                    </tr>
                </tbody>
            </table>
