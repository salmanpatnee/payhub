...
defineProps<{ vivaAccounts: VivaAccountRow[] }>();
...
const deactivateTarget = ref<VivaAccountRow | null>(null);
const deactivateOpen   = ref(false);
const deactivateForm   = useForm({});

const activateTarget = ref<VivaAccountRow | null>(null);
const activateOpen   = ref(false);
const activateForm   = useForm({});

const deleteTarget = ref<VivaAccountRow | null>(null);
const deleteOpen   = ref(false);
const deleteForm   = useForm({});

function confirmActivate(account: VivaAccountRow) {
...
function executeActivate() {
    ...
    activateForm.patch(`/admin/viva-accounts/${activateTarget.value.id}/activate`, {
        onSuccess: () => {
...
function confirmDeactivate(account: VivaAccountRow) {
...
function executeDeactivate() {
    ...
    deactivateForm.patch(`/admin/viva-accounts/${deactivateTarget.value.id}/deactivate`, {
        onSuccess: () => {
...
function confirmDelete(account: VivaAccountRow) {
...
function executeDelete() {
    ...
    deleteForm.delete(`/admin/viva-accounts/${deleteTarget.value.id}`, {
        onSuccess: () => {
...
<template>
    <Head title="Viva Accounts" />

    <div class="p-6 space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold tracking-tight">Viva Accounts</h1>
                <p class="text-sm text-muted-foreground mt-1">
                    Viva payments are GBP-only and redirect to Viva's hosted Smart Checkout page.
                </p>
            </div>
            <Button as-child>
                <Link href="/admin/viva-accounts/create">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-[#F7F5F2] border-b border-border">
                        ...
                        <th class="text-left px-5 py-3.5 text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">Client ID</th>
                ...
                <tbody>
                        ...
                        <td class="px-5 py-3.5 text-right">
                            <div class="flex items-center justify-end gap-1">
                                <Button variant="ghost" size="icon" as-child>
                                    <Link
                                        :href="`/admin/viva-accounts/${account.id}/edit`"
                    ...
                    <tr v-if="vivaAccounts.length === 0">
                        <td colspan="5" class="px-5 py-16 text-center text-muted-foreground text-sm">
                            No Viva accounts yet. Add an account to enable GBP payment collection via Viva.
                        </td>
                    </tr>
                </tbody>
            </table>
