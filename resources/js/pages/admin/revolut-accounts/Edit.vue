...
const props = defineProps<{ revolutAccount: RevolutAccountProp }>();

const form = useForm({
    _method:        'PUT',
    account_name:   props.revolutAccount.account_name,
...
async function copyEndpointUrl(): Promise<void> {
    try {
        await navigator.clipboard.writeText(props.revolutAccount.webhook_endpoint_url);
    } catch {
        const el = document.createElement('textarea');
        el.value = props.revolutAccount.webhook_endpoint_url;
...
function testConnection() {
    ...
    const url = form.secret_key
        ? '/admin/revolut-accounts/test-connection'
        : `/admin/revolut-accounts/${props.revolutAccount.id}/test-connection`;
    const data = form.secret_key ? { secret_key: form.secret_key } : {};
...
function submit() {
    form.post(`/admin/revolut-accounts/${props.revolutAccount.id}`);
}
</script>

<template>
    <Head :title="`Edit ${revolutAccount.account_name}`" />
    ...
    <div class="p-6">
        ...
        <Card>
            <CardHeader>
                <CardTitle>Edit Revolut account</CardTitle>
                <CardDescription>
                ...
                <form id="edit-account-form" class="space-y-4" @submit.prevent="submit">
                    ...
                    <div class="grid gap-2">
                        ...
                        <div class="flex items-center gap-2 rounded-lg border border-border bg-muted/40 px-4 py-3">
                            <span class="flex-1 truncate select-all font-mono text-sm">{{ revolutAccount.webhook_endpoint_url }}</span>
                        ...
                        <p class="text-xs text-muted-foreground">
                            Register this URL in the Revolut dashboard when creating a webhook.
                        </p>
                    </div>

                    <!-- Webhook signing secret (masked, blank = preserve) -->
                    <div class="grid gap-2">
                        <Label for="webhook_secret">Webhook signing secret</Label>
                        <div
                            v-if="revolutAccount.has_webhook_secret"
                        ...
                        />
                        <p class="text-xs text-muted-foreground">
                            Leave blank to keep the current secret. Paste a new value (starts with wsk_) to replace it.
                        </p>
