                ...
                <CardContent>
                    <dl class="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                        ...
                        <div v-if="payment.provider_reference" class="col-span-2">
                            <dt class="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-foreground/60">
                                <Hash class="size-3.5 shrink-0" />{{ payment.provider === 'revolut' ? 'Revolut Order' : payment.provider === 'square' ? 'Square Payment ID' : payment.provider === 'viva' ? 'Viva Order/Transaction ID' : payment.provider === 'clover' ? 'Clover Payment ID' : 'Stripe PI' }}
                            </dt>
