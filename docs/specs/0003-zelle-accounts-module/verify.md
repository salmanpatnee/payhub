...
## UI / manual
- [x] Log in as admin, open Zelle Accounts, Add account with name, email `Pay@Acme.test`, mobile, USD → saved, listed, email shown lowercase → AC-1, AC-2
- [x] Add a second account with the same email in different case → error on email → AC-2
- [x] Delete the first account, then re-add the same email → allowed → AC-2
- [x] Enter `555-CALL` as mobile → error → AC-3
- [x] Edit, deactivate, activate, delete from the list as admin and as the account role user → all work → AC-4, AC-5
- [x] Assign an agent on Create/Edit; only agents appear in the picker → AC-6
...
## Commands
- [x] `php artisan test tests/Feature/ZelleAccountManagementTest.php` → all pass → AC-1 to AC-12
- [x] `SELECT COUNT(*) FROM activity_logs` before and after the manual steps → unchanged → AC-12
